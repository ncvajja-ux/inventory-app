...
invDb.serialize(() => {
    ...
    // Migrate: add tax_category to mara if missing
    invDb.all("PRAGMA table_info(mara)", [], (err, cols) => {
        if (err || !cols) return;
        if (!cols.find(c => c.name === 'tax_category')) {
            invDb.run("ALTER TABLE mara ADD COLUMN tax_category TEXT",
    ...
    transDb.all("PRAGMA table_info(vbap)", [], (err, cols) => {
        ...
        toAdd.forEach(([col, def]) => {
            if (!names.includes(col)) {
                transDb.run(`ALTER TABLE vbak ADD COLUMN ${col} ${def}`,
...
pricingDb.serialize(() => {
    ...
    // Seed default GST slabs if empty
    pricingDb.get("SELECT COUNT(*) as n FROM gst_config", [], (err, row) => {
        if (err || (row && row.n > 0)) return;
        const slabs = [
            ['Apparel',     12.00],
        ...
        const stmt = pricingDb.prepare("INSERT OR IGNORE INTO gst_config (tax_category,gst_rate) VALUES (?,?)");
        slabs.forEach(([cat, rate]) => stmt.run(cat, rate));
        stmt.finalize(() => console.log("✅ GST config seeded"));
    });
});
...
app.post("/categories", (req, res) => {
    ...
    invDb.run("INSERT INTO categories (category,subcategory) VALUES (?,?)", [category,subcategory],
...
// PUT update PO line status — triggers inventory update on Goods Receipt
...
// GET active GST rate for a tax_category (used at order time)
...
// GET full order history for a customer (sales + returns)
...
// GET purchase order history for a product
...
// ─── Return Order Routes ──────────────────────────────────────────────────────
// GET orders by customer + date range (for finding original order)
app.get("/orders/search-for-return", (req, res) => {
    ...
    // Include both S-prefixed and old numeric orders
    ...
    transDb.all(sql, params, (err, rows) => {
        if (err) return res.status(500).json({error:err.message});
        if (!rows.length) return res.json([]);
        // Enrich with customer name from custDb
        let pending = rows.length;
        const enriched = [];
        rows.forEach(row => {
            custDb.get("SELECT name FROM kna1 WHERE kunnr=?", [row.kunnr], (err, cust) => {
    ...
    nextId(transDb, "vbak", "order_id", "R", (err, order_id) => {
        ...
        transDb.serialize(() => {
            ...
            transDb.run("COMMIT", (err) => {
                ...
                // Update stock — return adds back to quantity
                items.forEach(item => {
